...
Байндинг, связывание которого производится только от источника (source) к приёмнику (target). В качестве source у нас выступает всегда dataContext, поэтому в следующем примере связывание работает из dataContext в GroupBox.
```xml
...
```xml
<GroupBox Title="{Binding Path=GroupBoxTitle, Mode=OneWay}"/>
```
...
- Не поддерживается свойство `Path` - думаю, в будущем, появится и это, для этого достаточно немного дописать код расширения разметки.