    ...
    public class WindowsHost : Control
    {
        ...
        private void activateWindow(Window window) {
            ...
            if ( ConsoleApplication.Instance.FocusManager.CurrentScope != window ) {
                ...
                // Текущая реализация системы размещения не определяет автоматически
                // по изменениям в Z-Order контролов, какие области необходимо перерисовать.
                // Поэтому в этом месте мы вызываем Invalidate() для окна, выходящего наверх,
                // чтобы его содержимое было полностью обновлено на экране, вместе с дочерними элементами,
                // даже несмотря на то, что скорее всего они не изменились.
                // Если убрать этот вызов и поэкспериментировать с размещением окон друг над другом,
                // можно легко заметить, что при активации нижнего окна будут перерисованы только
                // те элементы, которые изменились при получении окном фокуса (или всё окно,
                // если была тронута рамка в местах перемещения или ресайза).
                // todo : описать это в документации, предложить полное решение проблемы
                window.Invalidate(  );
            }
        }
