    ...
    public class WindowsHost : Control
    {
        ...
        private void activateWindow(Window window) {
            ...
            if ( ConsoleApplication.Instance.FocusManager.CurrentScope != window ) {
                ...
                // Поэтому в этом месте мы вызываем Invalidate() для WindowsHost'а целиком,
                // чтобы его содержимое было полностью обновлено на экране, вместе с окнами,
                // даже несмотря на то, что скорее всего эти окна внутри не изменились.
